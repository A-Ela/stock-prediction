...
export default function NotificationsPage() {
  ...
  return (
    <div>
      ...
      {notes.map((n, i) => {
        const type = n.message?.toLowerCase().includes("dropped") || n.message?.toLowerCase().includes("down") ? "down" : "up";
        return (
        <button
          key={i}
          onClick={() => handleMarkRead(n._id, n.isRead)}
          style={{ width: "100%", padding: 18, borderRadius: 10, marginBottom: 12, background: n.isRead ? COLORS.bg2 : `linear-gradient(90deg, ${type === "up" ? COLORS.green + "0A" : COLORS.red + "0A"}, ${COLORS.bg2})`, border: `1px solid ${n.isRead ? COLORS.border : type === "up" ? COLORS.green + "33" : COLORS.red + "33"}`, display: "flex", alignItems: "flex-start", gap: 14, textAlign: "left", cursor: n.isRead ? "default" : "pointer" }}
        >
          <div style={{ width: 36, height: 36, borderRadius: 8, flexShrink: 0, background: type === "up" ? `${COLORS.green}20` : `${COLORS.red}20`, border: `1px solid ${type === "up" ? COLORS.green + "44" : COLORS.red + "44"}`, display: "flex", alignItems: "center", justifyContent: "center", color: type === "up" ? COLORS.green : COLORS.red, fontSize: 14 }}>{type === "up" ? "▲" : "▼"}</div>
          <div style={{ flex: 1 }}>
